...
def get_rotten_tomatoes_html(movie_url):
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                "Failed to fetch data from Rotten Tomatoes. Status code:",
...
def get_movie_info(html_code):
    ...
    for info_item in info_list.find_all("li", {"data-qa": "movie-info-item"}):
        ...
        value = info_item.find(
            "span", {"data-qa": "movie-info-item-value"}
        ).text.strip()
...
def main():
    ...
    print(reviews_df)
    # reviews_df = reviews_df.rename(columns={"Director:": "Director"})
    reviews_df["Genre:"] = reviews_df["Genre:"].apply(
        lambda x: [word.strip() for word in x.split()]
    )
    reviews_df["Genre"] = reviews_df["Genre:"].apply(
        lambda x: x[0][:-1].strip() if x and x[0] else None
    )
    ...
    filter_reviews_df.to_excel("reviews.xlsx", index=False)
